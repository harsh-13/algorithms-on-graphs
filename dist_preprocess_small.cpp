...
Queue order_pq;
vector<ll> outgoing[MAX];                      
vector<ll> incoming[MAX];                     
vector<ll> out_cost[MAX];                     
vector<ll> in_cost[MAX];                      
ll order[MAX];
vector<ll> contracted_neighbour(MAX,0);       
vector<bool> contracted(MAX, 0);              
set<ll> workset;                               //workset for storing the work done on which vertices
ll n, m;
vector<ll> dist_u(MAX, INF);                   // distance from vertice u in dijkstras;
vector<ll> hops_u(MAX, 0);                     // stores the number of hops from u
...
// dijkstras to check for witness path
void dijkstras(ll u, ll max_tot) {
	//1- create pq which stores the {dist, vertices} format and distacne from the u vertices;
	     ...
	     while (!pq.empty()) {
		         ...
		         visited[u1] = 1;
		         workset.insert(u1);
		         if (dist_u[u1] > max_tot) break; // stop witness path search as it crossed max path 
...
void contract(ll v) {
	// calculate the maximum incoming and outgoing from the vertices v ie. max(l(u,v) + l(v,w))
	// to stop the witness path to go further if witness path found more the sum of max_in + max_out
	// now run dijkstras over each of the incoming vertices coming at v and try to find the witness path is there or not 
	     contracted[v] = 1;
	     ll max_in = 0;
	     ll max_out = 0;
...
void preprocess() {
	...
	// for this try to find the witness path
	     ...
	     while (!order_pq.empty()) {
			     ...
			     if (new_imp > imp) {   
			     	//importnce change becuse of neighbour is contracted and we don't want to contract the neighbour which are close to each other
